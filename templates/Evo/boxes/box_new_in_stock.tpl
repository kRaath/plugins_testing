{if isset($oBox->Artikel->elemente)}
    ...
    {include file='snippets/product_slider.tpl' id='boxslider-newproducts' productlist=$oBox->Artikel->elemente title=$slidertitle tplscope='box' moreLink=$moreLink moreTitle=$moreTitle}
    {*
...
{elseif isset($Boxen.NeuImSortiment)}
    {lang key="newProducts" section="global" assign='slidertitle'}
    {include file='snippets/product_slider.tpl' id='boxslider-newproducts' productlist=$Boxen.NeuImSortiment->Artikel->elemente title=$slidertitle tplscope='box'}
    {*
    <a href="{$Boxen.NeuImSortiment->cURL}" class="artikelnamelink">{lang key="showAllNewProducts" section="global"}</a>
    *}
{/if}