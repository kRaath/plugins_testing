		...
		obXHR.onreadystatechange = function() {
			if (obXHR.readyState == 1) {
				...
				if (obTam.length > 0)
				{
					...
					//document.getElementById("amountCalc").innerHTML = obTcu[0].firstChild.firstChild.nodeValue + " " + formatNum(obTam[0].firstChild.nodeValue, 2,',','.','','','-','');
				...
				else
				{
					...
					//document.getElementById("amountCalc").innerHTML = document.getElementById("slcToCurrency").value + " " + formatNum(document.getElementById("txtAmount").value, 2,',','.','','','-','');
				}
			}
		}
		obXHR.send(null);
