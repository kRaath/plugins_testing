...
class xajaxResponse
{
    ...
    */
    public function __construct()
    {
        ...
        if (0 < func_num_args()) {
            ...
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:EDERR:01'), E_USER_ERROR
                    );
        }
        //EndSkipDebug
    ...
    */
    public function &plugin()
    {
        ...
        if (false == (0 < $nArgs)) {
            $objLanguageManager = xajaxLanguageManager::getInstance();
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:MPERR:01'), E_USER_ERROR
                    );
        }
        //EndSkipDebug
    ...
    */
    ...
    /*
        ...
        object : The <xajaxResponse> object.
    */
    public function create($sParent, $sTag, $sId, $sType=null)
    {
        //SkipDebug
        if (false === (null === $sType)) {
            $objLanguageManager = xajaxLanguageManager::getInstance();
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:CPERR:01'), E_USER_WARNING
                    );
        }
        //EndSkipDebug
    ...
    */
    public function loadCommands($mCommands, $bBefore=false)
    {
        if (is_a($mCommands, 'xajaxResponse')) {
            ...
            //SkipDebug
                if (!empty($mCommands)) {
                    $objLanguageManager = xajaxLanguageManager::getInstance();
                    trigger_error(
                            $objLanguageManager->getText('XJXRSP:LCERR:01'), E_USER_ERROR
                            );
                }
                //EndSkipDebug
        }
    ...
    public function absorb($objResponse)
    {
    ...
    */
    public function _printHeader_XML()
    {
        echo '<';
        echo '?';
    ...
    */
    ...
    /*
        ...
        aAttributes - (array):  Associative array of attributes for this
            command.
    */
    public function _printCommand_XML(&$aAttributes)
    {
        echo '<';
        echo 'cmd';
    ...
    */
    public function _printArray_XML(&$mArray)
    {
            ...
            if (is_array($mArray[$sKey])) {
                ...
                foreach (array_keys($mArray[$sKey]) as $sInnerKey) {
                    ...
                    if (htmlspecialchars($sInnerKey, ENT_COMPAT, 'UTF-8') != $sInnerKey) {
                        $objLanguageManager = xajaxLanguageManager::getInstance();
                        trigger_error(
                                $objLanguageManager->getText('XJXRSP:AKERR:01'), E_USER_ERROR
                                );
                    }
                    //EndSkipDebug
                    ...
                    if ('k' == $sInnerKey || 'v' == $sInnerKey) {
                        ...
                        $objLanguageManager = xajaxLanguageManager::getInstance();
                        trigger_error(
                                $objLanguageManager->getText('XJXRSP:IEAERR:01'), E_USER_ERROR
                                );
                        //EndSkipDebug
                    }
                }
                ...
                $objLanguageManager = xajaxLanguageManager::getInstance();
                trigger_error(
                        $objLanguageManager->getText('XJXRSP:NEAERR:01'), E_USER_ERROR
                        );
                //EndSkipDebug
            }
        }
    ...
    */
    public function _printEscapedString_XML(&$sData)
    {
        ...
        if ($this->bOutputEntities) {
            ...
            if (false === function_exists('mb_convert_encoding')) {
                $objLanguageManager = xajaxLanguageManager::getInstance();
                trigger_error(
                        $objLanguageManager->getText('XJXRSP:MBEERR:01'), E_USER_NOTICE
                        );
            }
            //EndSkipDebug
        ...
        if (0 < $nCDATA) {
            ...
            if (is_string($sData)) {
                echo 'S';
            } elseif (is_int($sData) || is_float($sData)) {
                echo 'N';
            } elseif (is_bool($sData)) {
                echo 'B';
            }
    ...
    */
    public function _encodeArray(&$mData)
    {
...
class xajaxCustomResponse
{
    ...
    public function absorb($objResponse)
    {
        ...
        if (false == is_a($objResponse, 'xajaxCustomResponse')) {
            $objLanguageManager = xajaxLanguageManager::getInstance();
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:MXRTERR'), E_USER_ERROR
                    );
        }
        ...
        if ($objResponse->getContentType() != $this->getContentType()) {
            $objLanguageManager = xajaxLanguageManager::getInstance();
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:MXCTERR'), E_USER_ERROR
                    );
        }
        ...
        if ($objResponse->getCharacterEncoding() != $this->getCharacterEncoding()) {
            $objLanguageManager = xajaxLanguageManager::getInstance();
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:MXCEERR'), E_USER_ERROR
                    );
        }
        ...
        if ($objResponse->getOutputEntities() != $this->getOutputEntities()) {
            $objLanguageManager = xajaxLanguageManager::getInstance();
            trigger_error(
                    $objLanguageManager->getText('XJXRSP:MXOEERR'), E_USER_ERROR
                    );
        }
        //EndSkipDebug
