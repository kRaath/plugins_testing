...
if (!defined('XAJAX_METHOD_UNKNOWN')) {
    define('XAJAX_METHOD_UNKNOWN', 0);
}
if (!defined('XAJAX_METHOD_GET')) {
    define('XAJAX_METHOD_GET', 1);
}
if (!defined('XAJAX_METHOD_POST')) {
    define('XAJAX_METHOD_POST', 2);
}
...
class xajaxArgumentManager
{
    ...
    */
    ...
    public function convertStringToBool($sValue)
    {
    ...
    public function argumentStripSlashes(&$sArg)
    {
    ...
    public function argumentDecodeXML(&$sArg)
    {
        if (false == is_string($sArg)) {
            return;
        }
...
//				$sText .= $sArg[$nCurrent];
    ...
    public function argumentDecodeUTF8_iconv(&$mArg)
    {
        if (is_array($mArg)) {
    ...
    public function argumentDecodeUTF8_mb_convert_encoding(&$mArg)
    {
        if (is_array($mArg)) {
            foreach (array_keys($mArg) as $sKey) {
                $sNewKey = $sKey;
    ...
    public function argumentDecodeUTF8_utf8_decode(&$mArg)
    {
        if (is_array($mArg)) {
            foreach (array_keys($mArg) as $sKey) {
                $sNewKey = $sKey;
    ...
    */
    public function configure($sName, $mValue)
    {
        ...
        } elseif ('characterEncoding' == $sName) {
    ...
    */
    public function process()
    {
        if ($this->bDecodeUTF8Input) {
            ...
            } elseif ($this->sCharacterEncoding == "ISO-8859-1") {
                $sFunction = "utf8_decode";
            } else {
                $objLanguageManager = xajaxLanguageManager::getInstance();
                trigger_error(
                    $objLanguageManager->getText('ARGMGR:ERR:03'), E_USER_NOTICE
                    );
            }
