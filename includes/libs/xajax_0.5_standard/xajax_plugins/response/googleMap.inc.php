...
class clsGoogleMap extends xajaxResponsePlugin
{
    ...
    */
    ...
    public function setGoogleSiteKey($sKey)
    {
        $this->sGoogleSiteKey = $sKey;
    }
    
    public function create($sMap, $sParentId)
    {
        $command = array('n'=>'gm:cr', 't'=>$sParentId);
        $this->addCommand($command, $sMap);
    }
    public function zoom($sMap, $nZoom)
    {
        $command = array('n'=>'gm:zm', 't'=>$sMap);
        $this->addCommand($command, $nZoom);
    }
    public function setMarker($sMap, $nLat, $nLon, $sText)
    {
        $this->addCommand(
    ...
    public function moveTo($sMap, $nLat, $nLon)
    {
        //	39.928005,
//	-82.70784,
//	15);
