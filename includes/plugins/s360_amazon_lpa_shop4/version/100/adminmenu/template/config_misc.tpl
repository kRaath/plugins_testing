...
<div class="panel panel-default">
    <div class="panel-heading"><h3>Versandarten-Ausschluss</h3></div>
    <div class="panel-body">
        <form method="post" id="lpa-account-settings-form" action="{$pluginAdminUrl}cPluginTab=Einstellungen%20Sonstiges">
            ...
            {foreach item=deliverymethod from=$s360_lpa_config_misc.lpa_available_delivery_methods}
                <div class="col-xs-12">
                <input type="checkbox" name="lpa_excluded_delivery_methods[]" value="{$deliverymethod.key}" {if $deliverymethod.isExcluded} checked{/if}> {$deliverymethod.name}
                </div>
            {/foreach}
