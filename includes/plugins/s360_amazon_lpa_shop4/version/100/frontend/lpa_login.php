...
if ($amazonName) {
    ...
    Shop::Smarty()->assign('lpa_last_name', $formLastName);
}
...
if (empty($result)) {
    ...
    if (isset($_SESSION['Kunde']) && $_SESSION['Kunde']->kKunde) {
         ...
         */
        $sql = 'SELECT * FROM tkunde WHERE cMail LIKE "' . $amazonEmail . '"';
        ...
        if (empty($kunde)) {
             ...
             */
            if (count($kunde) > 1) {
                Jtllog::writeLog('LPA: LPA-Login-Fehler: Es wurde mehr als ein Kunde mit der E-Mail-Adresse "' . $amazonEmail . '" im Shop gefunden. Es kann keine Account-Zuordnung durchgeführt werden! Prüfen Sie bitte den Datenbestand!', JTLLOG_LEVEL_ERROR);
                $displayMode = 'error';
            } else {
                $kunde = $kunde[0];
    ...
    if (isset($_SESSION['Kunde']) && $_SESSION['Kunde']->kKunde) {
         ...
         */
        $kunde = new Kunde();
        $kunde = $kunde->holRegKundeViaEmail($amazonEmail);
        if ($kunde && intval($kunde->kKunde) === intval($result->kKunde)) {
            /*
            ...
            // the user referenced in our mapping does not exist in the database, at least not with the given kKunde, therefore the mapping must be considered
            ...
            $sql = 'SELECT * FROM tkunde WHERE cMail LIKE "' . $amazonEmail . '" LIMIT 1';
...
$mergeUrl = str_replace("http://", "https://", URL_SHOP) . '/lpamerge';
