...
Shop::Smarty()->assign('lpa_charge_on_order', $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_ADVANCED_CAPTUREMODE]);
...
Shop::Smarty()->assign('PluginFrontendUrl', $oPlugin->cFrontendPfadURLSSL);
...
Shop::Smarty()->assign('lpaEinstellungen', Shop::getSettings(array(CONF_GLOBAL, CONF_RSS, CONF_KUNDEN, CONF_KAUFABWICKLUNG, CONF_KUNDENFELD, CONF_TRUSTEDSHOPS)));
...
if ($lpa_step === 'lpaselected' && !empty($_SESSION['Versandart'])) {
    ...
    Shop::Smarty()->assign('lpa_orid', $orid);

    Shop::Smarty()->assign('AGB', gibAGBWRB($_SESSION['kSprache'], $_SESSION['Kundengruppe']->kKundengruppe));

    // Assign smarty constants thay *may* be used by the overview of ordered articles
    Shop::Smarty()->assign('C_WARENKORBPOS_TYP_ARTIKEL', C_WARENKORBPOS_TYP_ARTIKEL);
    Shop::Smarty()->assign('C_WARENKORBPOS_TYP_GRATISGESCHENK', C_WARENKORBPOS_TYP_GRATISGESCHENK);
