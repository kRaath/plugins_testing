...
try {
    ...
    Shop::Smarty()->assign('lpa_login_redirect_uri', Shop::getURL(true) . '/lpalogin'); // MUST BE SSL!
     ...
     */
    $lpa_other_urls['checkout'] = Shop::getURL(true) . '/lpacheckout';
    $lpa_language_code = "de-DE";

    /*
     * Complete URL for redirect after order completion - localized!
     */
    $completeUrlLocalized = str_replace("http://", "https://", Shop::getURL()) . '/lpacomplete';
    if (Shop::getLanguage(true) === "eng") {
        $completeUrlLocalized .= '-en';
     ...
     */
    /* Get insertion position for jQuery-using scripts */
    $jsPosition = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_ADVANCED_JSPOSITION];
    ...
    if ($mode !== 'p') {
        $pqMethod = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_LOGINBUTTON_INSERT_METHOD_MOBILE];
        $pqSelectors = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_LOGINBUTTON_INSERT_SELECTORS_MOBILE];

        $insertInParentMode = false;
        if (empty($pqSelectors)) {
            $pqSelectors = S360_LPA_SELECTOR_LOGIN_INPUT_MOBILE;
            $pqMethod = 'after';
        ...
        if ($validCurrency && isset($_SESSION['Warenkorb']) && count($_SESSION['Warenkorb']->PositionenArr) > 0) {
            ...
            foreach ($_SESSION['Warenkorb']->PositionenArr as $oPosition) {
                if ((int) $oPosition->nPosTyp === (int) C_WARENKORBPOS_TYP_ARTIKEL && is_object($oPosition->Artikel)) {
                    if (isset($oPosition->Artikel->FunktionsAttribute['exclude_amapay'])) {
                        $bExclude = true;
                        break;
                    }
                }
            ...
            if ($bExclude) {
                 ...
                 * add pay with amazon
                 */
                $pqMethod = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_INSERT_METHOD_MOBILE];
                $pqSelectors = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_INSERT_SELECTORS_MOBILE];
                if (empty($pqSelectors)) {
                ...
                Shop::Smarty()->assign('lpa_login_redirect_uri', Shop::getURL(true) . '/lpacheckout'); // MUST BE SSL!
                ...
                if (Shop::getPageType() === PAGE_BESTELLVORGANG) {
                    ...
                    if ($step === 'accountwahl') {
                        ...
                        $configBestellschritte = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_DURING_CHECKOUT_MOBILE];
                    ...
                    if ($showDuringCheckout) {
                        ...
                        $pqMethod = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_DURING_CHECKOUT_INSERT_METHOD_MOBILE];
                        $pqSelectors = $oPlugin->oPluginEinstellungAssoc_arr[S360_LPA_CONFKEY_PAYBUTTON_DURING_CHECKOUT_INSERT_SELECTORS_MOBILE];

                        if (empty($pqSelectors)) {
                            // default to standard selectors if no selector is selected (else, pq() will add the element before the HTML)
                            $pqSelectors = S360_LPA_SELECTOR_PAY_BUTTON_DURING_CHECKOUT_MOBILE;
                        }
