...
switch ($oPlugin->nCalledHook)
{
    case HOOK_SMARTY_OUTPUTFILTER:  //Hook140
        ...
        if(count($queryResult) == 1)
        {
            ...
            if ( $helper->gibSeiten__Typ() == PAGE_ARTIKEL && $queryResult->bTS_ProductStickerShow == 1)
            {
                ...
                } elseif ($oArtikel_tmp->nIstVater == 1 && $queryResult->iTS_ProductStickerArt == 1)  {
                    $smarty->assign("agws_ts_features_showtab","on");

                    $sql = "SELECT cArtNr FROM `tartikel` WHERE kVaterArtikel = ".$oArtikel_tmp->kArtikel;
                    ($helper->isShop4()) ?
            ...
            if ( ($helper->gibSeiten__Typ() == PAGE_BESTELLABSCHLUSS || $helper->gibSeiten__Typ() == PAGE_BESTELLSTATUS) && isset($_SESSION['agws_kWarenkorb_TS']) && $_SESSION['agws_kWarenkorb_TS'] > 0)
            {
                $ts_checkout_pq_selector =TS_CHECKOUT_PQ_SELECTOR;
                $ts_checkout_pq_method = TS_CHECKOUT_PQ_METHOD;

                $sql="select kBestellung from tbestellung where kWarenkorb='".$_SESSION['agws_kWarenkorb_TS']."'";
                ($helper->isShop4()) ?
                    $agws_kWarenkorb_TS = Shop::DB()->query($sql,1):
                    $agws_kWarenkorb_TS = $GLOBALS["DB"]->executeQuery($sql,1);

                $sql="select cMail from tkunde where kKunde='".$_SESSION['agws_kKunde_TS']."'";
                ($helper->isShop4()) ?
                    $agws_kKunde_TS = Shop::DB()->query($sql,1):
    ...
    case HOOK_LETZTERINCLUDE_INC: //hook 99
    ...
    case HOOK_TOOLSAJAXSERVER_PAGE_TAUSCHEVARIATIONKOMBI: //hook 45
        ...
        if (count($queryResult) == 1 && $queryResult->cTS_ID != "")
        {
            ...
            $args_arr['objResponse']->script('ts_article_review_init("'.addslashes($agws_ts_features_ArtNr).'", "'.addslashes($agws_ts_features_TSID).'", "'.addslashes($agws_ts_features_Intro).'");');
        }
