...
{
    if (isset($_POST['ts_id']) && $helper->filter__XSS($_POST['ts_id']) != "" && isset($_POST['ts_id_is_add']) && $helper->filter__XSS($_POST['ts_id_is_add'])==1)
	{
		$ts_id_add = new stdClass();
		$ts_id_add->cTS_ID = $helper->filter__XSS($_POST['ts_id']);
        $ts_id_add->iTS_Sprache = $helper->filter__XSS($_POST['ts_sprache']);
    ...
    if (isset($_POST['ts_id']) && $helper->filter__XSS($_POST['ts_id']) != "" && isset($_POST['ts_id_is_delete']) && $helper->filter__XSS($_POST['ts_id_is_delete'])==1)
    {
        ($helper->isShop4()) ?
            $queryResult = Shop::DB()->deleteRow("xplugin_agws_ts_features_config", "cTS_ID",$helper->filter__XSS($_POST['ts_id'])):
            $queryResult = $GLOBALS["DB"]->deleteRow("xplugin_agws_ts_features_config", "cTS_ID",$helper->filter__XSS($_POST['ts_id']));

        if($queryResult==1)
        {
            $smarty->assign('ts_message','Die ID: '.$helper->filter__XSS($_POST['ts_id']).' wurde gelöscht!');
            $smarty->assign('ts_message_class','box_success');
        } else {
    ...
    if (isset($_POST['ts_id']) && $helper->filter__XSS($_POST['ts_id']) != "" && isset($_POST['ts_id_options_cancel']) && $helper->filter__XSS($_POST['ts_id_options_cancel'])==1)
    {
    ...
    if (isset($_POST['ts_id']) && $helper->filter__XSS($_POST['ts_id']) != "" && isset($_POST['ts_id_options_save']) && $helper->filter__XSS($_POST['ts_id_options_save'])==1)
    {
        $ts_id_edit = new stdClass();
        $ts_id_edit->cTS_ID = $helper->filter__XSS($_POST['ts_id']);
        $ts_id_edit->iTS_Sprache = $helper->filter__XSS($_POST['ts_sprache']);
        $ts_id_edit->cTS_BadgeCode = $_POST['ts_BadgeCode'];
        $ts_id_edit->bTS_RatingWidgetShow = $helper->filter__XSS($_POST['ts_RatingWidgetShow']);
        $ts_id_edit->iTS_RatingWidgetPosition = $helper->filter__XSS($_POST['ts_RatingWidgetPosition']);
        $ts_id_edit->bTS_ReviewStickerShow = $helper->filter__XSS($_POST['ts_ReviewStickerShow']);
        $ts_id_edit->iTS_ReviewStickerPosition = $helper->filter__XSS($_POST['ts_ReviewStickerPosition']);
        $ts_id_edit->bTS_RichSnippetsCategory = $helper->filter__XSS($_POST['ts_RichSnippetsCategory']);
        $ts_id_edit->bTS_RichSnippetsProduct = $helper->filter__XSS($_POST['ts_RichSnippetsProduct']);
        $ts_id_edit->bTS_RichSnippetsMain = $helper->filter__XSS($_POST['ts_RichSnippetsMain']);
        $ts_id_edit->cTS_ReviewStickerCode = $_POST['ts_ReviewStickerCode'];
        $ts_id_edit->bTS_ProductStickerShow = $helper->filter__XSS($_POST['ts_ProductStickerShow']);
        $ts_id_edit->iTS_ProductStickerArt = $helper->filter__XSS($_POST['ts_ProductStickerArt']);
        ...
        ($helper->isShop4()) ?
            ...
            $queryResult = $GLOBALS["DB"]->updateRow("xplugin_agws_ts_features_config", "cTS_ID", $ts_id_edit->cTS_ID, $ts_id_edit,1);
