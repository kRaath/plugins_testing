...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    if (isset($_POST['ts_id']) && filterXSS($_POST['ts_id']) != "" && isset($_POST['ts_id_is_add']) && filterXSS($_POST['ts_id_is_add'])==1)
	{
        ...
        $ts_id_add->iTS_Sprache = filterXSS($_POST['ts_sprache']);
...
{
    $smarty->assign('ts_message','Es trat ein Fehler auf - TS-ID ist bereits installiert oder es wurde keine Shop-Sprache ausgewählt!');
    $smarty->assign('ts_message_class','box_error');
}
...
$ts_id_sprache_free_arr = $GLOBALS["DB"]->executeQuery("SELECT * FROM tsprache WHERE kSprache NOT IN (SELECT iTS_Sprache FROM xplugin_agws_ts_features_config)",2);
