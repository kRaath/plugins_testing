                ...
                if (in_array($AktuellerArtikel->kArtikel, $oQueryTracking->nProduct_arr)) {
                    ...
                    if (intval($_POST['fragezumprodukt']) == 1) {
                        ...
                        if (pruefeIstVaterArtikel($AktuellerArtikel->kArtikel)) {// Varikombi
                            $kArtikelTmp = gibkArtikelZuVaterArtikel($AktuellerArtikel->kArtikel);
                        }
                        
                        $nHitType = JTLSEARCH_STAT_TYPE_BASKET;
                    }
                        
                    $cReturn = JtlSearch::doProductStats($oQueryTracking->kQuery, $kArtikelTmp, $nHitType, $oPlugin->getConf("cProjectId"), $oPlugin->getConf("cAuthHash"), $oPlugin->getConf("cServerUrl"));
