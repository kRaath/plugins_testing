...
if (strlen($oPlugin->getConf("cProjectId")) > 0 && strlen($oPlugin->getConf("cAuthHash")) > 0 && strlen($oPlugin->getConf("cServerUrl")) > 0) {
    ...
    if (isset($_SESSION['ExtendedJTLSearch']->bExtendedJTLSearch) && isset($_SESSION['ExtendedJTLSearch']->nCreateTime) && ($_SESSION['ExtendedJTLSearch']->nCreateTime + 300) > time() && $_SESSION['cISOSprache'] == $oPlugin->getConf("cCurrentISO")) {
        $args_arr['bExtendedJTLSearch'] = $_SESSION['ExtendedJTLSearch']->bExtendedJTLSearch;
    } else {
        $return = JtlSearch::doCheck($_SESSION['Kundengruppe']->kKundengruppe, $_SESSION['cISOSprache'], $_SESSION['Waehrung']->cISO, $oPlugin->getConf("cProjectId"), $oPlugin->getConf("cAuthHash"), urldecode($oPlugin->getConf("cServerUrl")));
        ...
        if (is_object($return)) {
            ...
            $GLOBALS['DB']->executeQuery("UPDATE tjtlsearchserverdata SET cValue = '{$return->_serverurl}' WHERE cKey = 'cServerUrl'", 3);
