...
if (isset($_SESSION['ExtendedJTLSearch']->bExtendedJTLSearch) && $_SESSION['ExtendedJTLSearch']->bExtendedJTLSearch &&
    ...
    if (isset($_SESSION['ExtendedJTLSearch']->oQueryTracking_arr) && is_array($_SESSION['ExtendedJTLSearch']->oQueryTracking_arr) && $nQueryTracking > 0) {
        if (Jtllog::doLog(JTLLOG_LEVEL_DEBUG)) {
            Jtllog::writeLog('Producthit mit oQueryTracking_arr: ' . print_r($_SESSION['ExtendedJTLSearch']->oQueryTracking_arr, true), JTLLOG_LEVEL_DEBUG, false, 'kPlugin', $oPlugin->kPlugin);
        }
        ...
        if ($oQueryTracking_arr !== null) {
            if (Jtllog::doLog(JTLLOG_LEVEL_DEBUG)) {
                Jtllog::writeLog('Producthit mit oQueryTracking_arr sorted: ' . print_r($_SESSION['ExtendedJTLSearch']->oQueryTracking_arr, true), JTLLOG_LEVEL_DEBUG, false, 'kPlugin', $oPlugin->kPlugin);
            }
                ...
                if (in_array($AktuellerArtikel->kArtikel, $oQueryTracking->nProduct_arr)) {
                    ...
                    if (Jtllog::doLog(JTLLOG_LEVEL_DEBUG)) {
                        if (is_object($cReturn)) {
                            Jtllog::writeLog('Producthit doProductStats mit Return: ' . print_r($cReturn, true), JTLLOG_LEVEL_DEBUG, false, 'kPlugin', $oPlugin->kPlugin);
                        } else {
