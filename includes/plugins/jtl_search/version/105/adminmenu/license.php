...
if (isset($oServerSettings->cProjectId) && strlen($oServerSettings->cProjectId) > 0 &&
    ...
    $oForm->addElement('cCode', 'textarea', 'Lizenzschlüssel: ', array('style' => 'width: 500px; height: 120px;', 'value' => $cLicenseKey));
    $oForm->addElement('btn_serverinfo', 'submit', '', array('value' => 'Lizenz speichern', 'class' => 'button orange'));
                ...
                if (is_array($cData_arr) && count($cData_arr) == 2) {
                    ...
                    if (isset($oResult->_serverurl) && strlen($oResult->_serverurl) > 0 && isset($oResult->_code) && $oResult->_code == 1) {
                        try {
                            ...
                            $oDB->insertRow($oObject, 'tjtlsearchserverdata');
                            unset($oObject);
                            $smarty->assign('bStartedTestperiod', true);
                        } catch (Exception $oEx) {
