...
class JTLSEARCH_Verwaltung_export extends JTLSEARCH_Verwaltung_Base
{
    ...
    public function generateContent($bForce = false)
    {
        if ($this->getIssetContent() === false || $bForce == true) {
            ...
            $this->setName('Export');
            $this->setContentVar('oResultImportStatus', $this->getImportStatus());
            $this->setContentVar('oResultImportHistory', $this->getImportHistory());
        }
    }
    ...
    {
        $oReturn = $this->getImportData("getimporthistory");
        return $oReturn;
    }
    
    private function getImportError()
    {
        $oReturn = $this->getImportData("getimporterror");
        return $oReturn;
    }
    
    private function getImportStatus()
    {
        $oReturn = $this->getImportData("getimportstatus");
        return $oReturn;
    }
