    ...
    if ($oForm->isValid()) {
        ...
        if (isset($oResult->_serverurl) && strlen($oResult->_serverurl) > 0 && isset($oResult->_code) && $oResult->_code == 1) {
            ...
            try {
                ...
                if ($oDB->getAsObject('SELECT kCron FROM tcron WHERE cName = "JTLSearchExportCron"') === NULL) {
                    $oCron = new stdClass();
                    $oCron->kKey = $oPlugin->kPlugin;
                    $oCron->cKey = 'kExportqueue';
                    $oCron->cTabelle = 'tjtlsearchexportqueue';
                    $oCron->cName = 'JTLSearchExportCron';
                    $oCron->cJobArt = 'JTLSearchExport';
                    $oCron->nAlleXStd = '168';
                    $oCron->dStart = date('Y-m-d H:i:s');
                    $oCron->dStartZeit = date('H:i:s');
                    if ($oDB->insertRow($oCron, 'tcron')) {
                        $oDebugger->doDebug(__FILE__ . ': Eintrag erfolgreich in Tabelle tcron eingefuegt.', JTLSEARCH_DEBUGGING_LEVEL_DEBUG);
                    }
                } else {
                    $oDebugger->doDebug(__FILE__ . ': Es besteht bereits ein Eintrag in der Tabelle tcron mit dem Namen "JTLSearchExportCron"', JTLSEARCH_DEBUGGING_LEVEL_DEBUG);
                }
            } catch (Exception $oEx) {
