...
class JTLSearchDB
{
    ...
    {
        $this->execSQL('CREATE TABLE IF NOT EXISTS `tjtlsearchexportqueue` (`kExportqueue` INT( 10 ) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY ,`nLimitN` INT( 10 ) UNSIGNED NOT NULL ,`nLimitM` INT( 10 ) UNSIGNED NOT NULL ,`nExportMethod` INT( 10 ) UNSIGNED NOT NULL ,`bFinished` BOOLEAN NOT NULL ,`bLocked` BOOLEAN NOT NULL ,`dStartTime` TIMESTAMP NOT NULL ,`dLastRun` TIMESTAMP NOT NULL ) ENGINE = MYISAM ;');
        $this->execSQL('CREATE TABLE IF NOT EXISTS `tjtlsearchserverdata` (`kId` INT( 10 ) UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY ,`cKey` VARCHAR( 255 ) NOT NULL ,`cValue` VARCHAR( 255 ) NOT NULL) ENGINE = MYISAM ;');
        $this->execSQL("CREATE TABLE IF NOT EXISTS `tjtlsearchdeltaexport` (`kId` int(10) NOT NULL, `eDocumentType` enum('product','manufacturer','category') NOT NULL, `bDelete` tinyint(4) DEFAULT '0', `dLastModified` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`kId`,`eDocumentType`)) ENGINE=MyISAM;");
    }
