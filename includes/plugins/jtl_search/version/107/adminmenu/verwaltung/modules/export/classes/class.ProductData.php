...
class ProductData implements IItemData
{
    ...
    public static function getItemKeys(JTLSearchDB $oDB, $nLimitN, $nLimitM)
    {
        ...
        $oRes = $oDB->getAsObject("
                SELECT
                    tartikel.kArtikel AS kItem
                FROM
                    tartikel
                LEFT JOIN tartikelattribut ON tartikelattribut.kArtikel = tartikel.kArtikel
                    AND tartikelattribut.cName = '" . JTLSEARCH_PRODUCT_EXCLUDE_ATTR . "'
        ...
        try {
            $oRes = $this->oDB->getAsObject("
                ...
                LEFT JOIN tartikelattribut ON tartikelattribut.kArtikel = tartikel.kArtikel
                    AND tartikelattribut.cName = '" . JTLSEARCH_PRODUCT_EXCLUDE_ATTR . "'
                WHERE
                    tartikelattribut.kArtikel IS NULL AND
                    tartikel.kArtikel = {$kItem}", 1);
            if ($oRes == false) {
    ...
    private function loadProductLanguagesFromDB()
    {
        $oArtikelSprache_arr = $this->oDB->getAsObject("
            SELECT
                tartikelsprache.kSprache, tartikelsprache.cName, tartikelsprache.cBeschreibung, tartikelsprache.cKurzBeschreibung, (SELECT cSeo FROM tseo WHERE  kKey = {$this->oProduct->kArtikel} AND kSprache = tartikelsprache.kSprache AND cKey = 'kArtikel' LIMIT 0, 1) as cSeo, tsprache.cShopStandard
            FROM
                tartikelsprache
            LEFT JOIN tsprache ON tartikelsprache.kSprache = tsprache.kSprache
            WHERE
                tartikelsprache.kArtikel = {$this->oProduct->kArtikel}", 2);
        ...
        $oArtikelAttr_arr = $this->oDB->getAsObject("
            SELECT
                kAttribut, cStringWert, cTextWert
            FROM
                tattribut
            WHERE
                kArtikel = {$this->oProduct->kArtikel} AND cName LIKE \"tab% inhalt\"", 2);
        ...
        if (is_array($oArtikelAttr_arr)) {
            foreach ($oArtikelAttr_arr as $oArtikelAttr) {
                ...
                //Andere Sprachen
                $oArtikelAttrSprache_arr = $this->oDB->getAsObject("
                    SELECT
                        kSprache, cStringWert, cTextWert
                    FROM
                        tattributsprache
                    WHERE
                        kAttribut = {$oArtikelAttr->kAttribut} AND (cStringWert != \"\" OR cTextWert != \"\")", 2);
                if (is_array($oArtikelAttrSprache_arr)) {
                    foreach ($oArtikelAttrSprache_arr as $oArtikelAttrSprache) {
                        if (strlen($oArtikelAttrSprache->cStringWert) > 0) {
