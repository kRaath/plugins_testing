...
if (verifyGPDataString('a') == 'delexport') {
    ...
    if ($oSecurity->createKey() == verifyGPDataString('p')) {
        ...
        //error_log(date('Y-m-d H:i:s') . ': cUrl: ' . substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1) . var_export(isset($cURL_arr['path']),true) . ";\n", 3, PFAD_ROOT.'jtllogs/jtlsearch_return.txt');
        if (isset($cURL_arr['path']) && strlen(substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1)) > 3) {
            if (file_exists(JTLSEARCH_PFAD_EXPORTFILE_DIR.substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1))) {
                ...
                if (unlink(JTLSEARCH_PFAD_EXPORTFILE_DIR.substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1))) {
                    ob_clean();
                    $oDebugger->doDebug(__FILE__.': Datei gelöscht. ('.JTLSEARCH_PFAD_EXPORTFILE_DIR.substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1).')', JTLSEARCH_DEBUGGING_LEVEL_NOTICE);
                } else {
                ...
                $oDebugger->doDebug(__FILE__.': Zu löschende Datei nicht vorhanden. ('.JTLSEARCH_PFAD_EXPORTFILE_DIR.substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1).')', JTLSEARCH_DEBUGGING_LEVEL_ERROR);
            }
        } else {
            $oDebugger->doDebug(__FILE__.': Kein Dateiname vorhanden oder zu kurz. ('.strlen(substr($cURL_arr['path'], strrpos($cURL_arr['path'], '/')+1)).')', JTLSEARCH_DEBUGGING_LEVEL_ERROR);
        }
    } else {
        $oDebugger->doDebug(__FILE__ . ': Konnte Datei nicht löschen! a: '. verifyGPDataString('a').'; p: '.verifyGPDataString('p').'; url: '. verifyGPDataString('url') .'; Security->createKey: '.$oSecurity->createKey(), JTLSEARCH_DEBUGGING_LEVEL_ERROR);
    }
    die('0');
    ...
    if ($oAccount->logged()) {
        ...
        if (preg_match("/^([0-3]{1}[0-9]{1}[.]{1}[0-1]{1}[0-9]{1}[.]{1}[0-9]{4}[ ]{1}[0-2]{1}[0-9]{1}[:]{1}[0-6]{1}[0-9]{1})/", $_POST['dStart'])) {
            ...
            $oDB->execSQL("UPDATE tcron SET dStart = FROM_UNIXTIME(".$dStart."), dLetzterStart = 0, dStartZeit = FROM_UNIXTIME(".$dStart.") WHERE cJobArt = 'JTLSearchExport'");
