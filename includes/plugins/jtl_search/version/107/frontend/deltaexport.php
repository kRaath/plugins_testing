...
$nDocumentCount = $GLOBALS['DB']->executeQuery('SELECT count(*) AS nCount FROM tjtlsearchdeltaexport', 1);
if ($nDocumentCount->nCount > 0) {
    // Alle Search-Einträge aus der Jobque schmeißen
    $GLOBALS['DB']->executeQuery('DELETE FROM tjobqueue WHERE cJobArt = "JTLSearchExport"', 3);
    $GLOBALS['DB']->executeQuery('UPDATE tjtlsearchexportqueue SET bFinished = 1 WHERE nExportMethod = 1', 3);
    
    
    if ($nDocumentCount->nCount <= JTLSEARCH_DELTAEXPORT_ITEMS_MAX) {
        $GLOBALS['DB']->executeQuery("INSERT INTO `tjobqueue` (`kCron` ,`kKey` ,`cJobArt` ,`cTabelle` ,`cKey` ,`nLimitN` ,`nLimitM` ,`nInArbeit` ,`dStartZeit`)
        ...
        unset($oTmpJobQueue);
        $GLOBALS['DB']->executeQuery('TRUNCATE TABLE tjtlsearchdeltaexport', 10);
    ...
    if (JTLSEARCH_DELTAEXPORT_TRIGGER_CRON) {
        $oJobQueueTMP = $GLOBALS['DB']->executeQuery('SELECT * FROM tjobqueue WHERE cJobArt = "JTLSearchExport" ORDER BY cTabelle ASC LIMIT 0, 1', 1);
        $oJobQueue = new JobQueue($oJobQueueTMP->kJobQueue, $oJobQueueTMP->kCron, $oJobQueueTMP->kKey, $oJobQueueTMP->nLimitN, $oJobQueueTMP->nLimitM, 0, $oJobQueueTMP->cJobArt, $oJobQueueTMP->cTabelle, $oJobQueueTMP->cKey, $oJobQueueTMP->dStartZeit, $oJobQueueTMP->dZuletztGelaufen);
