...
try {
    if (isset($args_arr['oArtikel']->kArtikel) && $args_arr['oArtikel']->kArtikel > 0) {
        ...
        if ($GLOBALS['DB']->executeQuery('UPDATE tjtlsearchdeltaexport SET bDelete = 0, dLastModified = now() WHERE kId = '.$oProductObj->kId.' AND eDocumentType = "'.$oProductObj->eDocumentType.'";', 3) == 0) {
            $GLOBALS['DB']->insertRow('tjtlsearchdeltaexport', $oProductObj);
        }
    }
} catch (Exception $oEx) {
