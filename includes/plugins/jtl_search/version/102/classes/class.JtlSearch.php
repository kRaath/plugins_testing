...
class JtlSearch
{
    ...
    public static function doSuggest($kKundengruppe, $cLanguageIso, $cCurrencyIso, $cValue, $cProjectId, $cAuthHash, $cServerUrl, &$oResponse)
    {
        ...
        try {
            $cResponse = Communication::postData(self::$cSearchURL . "searchdaemon/index.php", array("pid" => self::$cProjectId, "p" => $oSecurity->createKey(), "a" => self::$cAction, "q" => $cValue, "kdgrp" => $kKundengruppe, "lang" => $cLanguageIso, "curr" => $cCurrencyIso), true);
    ...
    public static function doSearch($cSessionID, $kKundengruppe, $cLanguageIso, $cCurrencyIso, $cValue, $cProjectId, $cAuthHash, $cServerUrl, $nProductsPerSite = 100, $nPage = 1, $bCheckStock = false, $cFilter = "", $cSort = "")
    {
        ...
        try {
            $cResponse = Communication::postData(self::$cSearchURL . "searchdaemon/index.php", array("pid" => self::$cProjectId, "p" => $oSecurity->createKey(), "a" => self::$cAction, "q" => $cValue, "rows" => $nProductsPerSite, "start" => $nStart, "filter" => $cFilter, "kdgrp" => $kKundengruppe, "lang" => $cLanguageIso, "curr" => $cCurrencyIso, "sessid" => $cSessionID, "sort" => $cSort, "instock" => $bCheckStock), true);
            ...
            if (count($oResponse->oSearch->oBanner_arr) > 0) {
                ...
                $GLOBALS['smarty']->assign('oImageMap', $oBannerShop3);
            }
    ...
    public static function doProductStats($kQuery, $kArtikel, $nHitType, $cProjectId, $cAuthHash, $cServerUrl)
    {
        ...
        try {
            $cResponse = Communication::postData(self::$cSearchURL . "searchdaemon/index.php", array("pid" => self::$cProjectId, "p" => $oSecurity->createKey(), "a" => self::$cAction, "query" => $kQuery, "product" => $kArtikel, "hittype" => $nHitType), true);
    ...
    public static function doSuggestForward($cQuery, $cLanguageIso, $cProjectId, $cAuthHash, $cServerUrl)
    {
        ...
        try {
            return Communication::postData(self::$cSearchURL . "searchdaemon/index.php", array("pid" => self::$cProjectId, "p" => $oSecurity->createKey(), "a" => self::$cAction, "query" => $cQuery, "langiso" => $cLanguageIso), true);
        } catch (Exception $exc) {
            ...
            foreach ($oResponse->oSearch->oFilterGroup_arr as $oFilterGroup) {
                ...
                if (isset($oFilterGroup->oFilterItem_arr) && is_array($oFilterGroup->oFilterItem_arr) && count($oFilterGroup->oFilterItem_arr) > 0) {
                    foreach ($oFilterGroup->oFilterItem_arr as $oFilterItem) {
                        ...
                        if ($oFilterGroup->cDataType == "float" && $oFilterGroup->nType == 2 && strpos($oFilterGroup->cName, "price") !== false) {// Price Slider
    ...
    public static function doCheck($kKundengruppe, $cLanguageIso, $cCurrencyIso, $cProjectId, $cAuthHash, $cServerUrl)
    {
        ...
        try {
            $cResponse = Communication::postData(self::$cSearchURL . "searchdaemon/index.php", array("pid" => self::$cProjectId, "p" => $oSecurity->createKey(), "a" => self::$cAction, "kdgrp" => $kKundengruppe, "lang" => $cLanguageIso, "curr" => $cCurrencyIso), true);
                    
            $oResponse = json_decode($cResponse);
            if (is_object($oResponse) && $oResponse->_code == 1) {
                return true;
            } elseif (is_object($oResponse) && ($oResponse->_code == 2 || $oResponse->_code == 3) && strlen($oResponse->_serverurl) > 0) {
                // Server change
                return $oResponse;
            } else {
                return false;
