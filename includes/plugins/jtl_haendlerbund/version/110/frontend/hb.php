...
{
    $nDelay = 600; // 10min Wartezeit zwischen den Versuchen.
    
    $oLatestTry = Shop::DB()->query("SELECT * FROM xplugin_jtl_haendlerbund_tupdate ORDER BY dVersuch DESC LIMIT 1", 1);
    
    
    $nDelay = pow($oLatestTry->nVersuch, 2)*60;
    if($nDelay > 86400) {
        $nDelay = 86400;  
    }

    
    $time = strtotime($oLatestTry->dVersuch);
    if ($time + $nDelay > time()) {
        Jtllog::writeLog('(haendlerbund) Aktualisierung pausiert aufgrund von Fehlern beim letzten Abruf. N&auml;chste Aktualisierung fr&uuml;hstens '.  date('d.m.Y H:i:s', $time + $nDelay), JTLLOG_LEVEL_DEBUG, false, 'kPlugin', $oPlugin->kPlugin);
        return;
    }    
        ...
        if (count($oUpdate_arr) > 0) {
            foreach ($oUpdate_arr as $oUpdate) {
                if (isset($oUpdate->kUpdate) && $oUpdate->kUpdate > 0) {
                    ...
                    // $nDelay
                    Shop::DB()->query("UPDATE xplugin_jtl_haendlerbund_tupdate 
                        SET dVersuch=NOW() WHERE kUpdate = " . intval($oUpdate->kUpdate), 3);
                    
                    // Updaten
                    if (!getNewHaendlerbund($oPlugin, $oUpdate->cType, $oUpdate->cFormat, $oUpdate->kUpdate, $oSprache->kSprache)) {
                        break;
                    }
                }
            }
