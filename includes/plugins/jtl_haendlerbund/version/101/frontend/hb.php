<?php
    ...
    {
        //Key -> Anzahl Fehlversuche, Value -> Intervall in Sek
        $nIntervallZeit_arr = array(0 => 60,
                                    3 => 120,
                                    5 => 600,
                                    10 => 1200,
                                    20 => 3600,
            ...
            if (count($oUpdate_arr) > 0) {
                foreach ($oUpdate_arr as $oUpdate) {
                    if (isset($oUpdate->kUpdate) && $oUpdate->kUpdate > 0) {
                        ...
                        if (isset($oUpdate->nVersuch) && $oUpdate->nVersuch > 0 && isset($oUpdate->dVersuch)) {
                            $oVersuchDatum = new DateTime($oUpdate->dVersuch);
                            $nIntervall = 0;
                            foreach ($nIntervallZeit_arr as $nKey => $nValue) {
                                if ($oUpdate->nVersuch > $nKey) {
                                    $nIntervall = $nValue;
                                } else {
                                    break;
                                }
                            }
                            if (time() < ($oVersuchDatum->getTimestamp()+$nIntervall)) {
                                Jtllog::writeLog("kUpdate (haendlerbund) " . $oUpdate->kUpdate . " wurde uebersprungen da Intervall nicht erreicht wurde!", JTLLOG_LEVEL_DEBUG, true, "kPlugin", $oPlugin->kPlugin);
                                continue;
                            }
                        }
                        // Updaten
