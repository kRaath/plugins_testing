...
class XML_GoogleShopping
{
    ...
    private function loadAvailibility($kArtikel)
    {
        if ($this->oExportArticle_arr[$kArtikel]->fLagerbestand > 0
            ...
            if ($this->oExportArticle_arr[$kArtikel]->fLagerbestand == 0 && $this->oExportArticle_arr[$kArtikel]->cLagerKleinerNull) {
                $this->oExportArticle_arr[$kArtikel]->cVerfuegbarkeit = 'available for order';
            } else {
    ...
    private function formatArticle($kArtikel)
    {
        if (isset($this->oExportArticle_arr[$kArtikel])) {
            ...
            $this->oExportArticle_arr[$kArtikel]->cBeschreibung = substr(str_replace($find, $replace, $this->oExportArticle_arr[$kArtikel]->cBeschreibung), 0, 9999);
