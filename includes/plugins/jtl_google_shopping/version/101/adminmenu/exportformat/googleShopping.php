...
/* ALT
$cDateiname_arr = explode('_', $exportformat->cDateiname);
$exportformat->cDateiname = $cDateiname_arr[0].'.xml';
*/
//NEU
...
$res = $GLOBALS["DB"]->executeQuery("SELECT tartikel.kArtikel
                                        FROM tartikel
...
$count = $GLOBALS["DB"]->executeQuery("SELECT count(tartikel.kArtikel) AS anzahl
                                        FROM tartikel
                                        LEFT JOIN tartikelattribut ON tartikelattribut.kArtikel = tartikel.kArtikel
                                                AND tartikelattribut.cName = '" . FKT_ATTRIBUT_KEINE_PREISSUCHMASCHINEN . "'
                                        " . $cSQL_arr['Join'] . "
...
if ($nLimitN+$nLimitM >= $count->anzahl) {
    ...
    if (class_exists('ZipArchive')) {
        $zip = new ZipArchive;
        if ($zip->open(PFAD_ROOT . PFAD_EXPORT . $exportformat->cDateiname,
                (is_file(PFAD_ROOT . PFAD_EXPORT . $exportformat->cDateiname)?null:ZIPARCHIVE::CREATE)) === true) {
    ...
    unlink(PFAD_ROOT . PFAD_EXPORT . $cTmpDateiname);
//Wenn MaxItem erreicht wurde dann XML-Foot schreiben und Datei einem Zip-Archiv hinzufügen
