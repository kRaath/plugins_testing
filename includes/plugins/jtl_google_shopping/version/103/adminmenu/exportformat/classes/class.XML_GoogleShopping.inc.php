        ...
        if (isset($oArticle_arr) && is_object($oArticle_arr) && $oArticle_arr->kArtikel > 0) {
            ...
            foreach ($this->oAttr_arr as $oAttr) {
                if ($oAttr->eWertHerkunft === 'VaterAttribut') {
                    ...
                    if (isset($oArticle_arr->cMerkmalAssoc_arr[strtolower($cWertName)]) && !empty($oArticle_arr->cMerkmalAssoc_arr[strtolower($cWertName)])) {
                        $cXML .= $this->writeAttribute($cPreAttribute, $oAttr->cGoogleName, $oArticle_arr->cMerkmalAssoc_arr[strtolower($cWertName)]);
                    }
                }
            }
