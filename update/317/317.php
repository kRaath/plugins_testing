<?php
    ...
    switch ($oVersion->nTyp) {
        ...
        case 2:
            function updKategorieLevel(array $kOberKategorie_arr = null, $nLevel = 1)
            {
                $nLevel = (int) $nLevel;

                $cSql = "WHERE kOberKategorie = 0";
                if ($kOberKategorie_arr === null) {
                    $GLOBALS['DB']->executeQuery("TRUNCATE tkategorielevel", 4);
                } else {
                    $cSql = "WHERE kOberKategorie IN (" . implode(",", $kOberKategorie_arr) . ")";
                }

                $oKategorie_arr = $GLOBALS['DB']->executeQuery("SELECT kKategorie, kOberKategorie
                                                                FROM tkategorie
                                                                {$cSql}", 2);
                ...
                if (count($oKategorie_arr) > 0) {
                    ...
                    foreach ($oKategorie_arr as $oKategorie) {
                        ...
                        $oKategorieLevel->kKategorie = $oKategorie->kKategorie;
                        $oKategorieLevel->nLevel = $nLevel;

                        $GLOBALS['DB']->insertRow("tkategorielevel", $oKategorieLevel);
                        unset($oKategorieLevel);
                    }

                    updKategorieLevel($kKategorie_arr, $nLevel + 1);
                }
            }
